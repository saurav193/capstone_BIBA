...
``` {r Figure 2, echo=FALSE, message=FALSE, fig.cap="Figure 2. Counts of features with a certain proportion of zeros."}
include_graphics("../results/report_figures/figure_2.png")
```

``` {r Figure 3, echo=FALSE, message=FALSE, fig.cap="Figure 3. Counts of features with a certain proportion of missing values"}
include_graphics("../results/report_figures/figure_3.png")
```

``` {r Figure 4, echo=FALSE, message=FALSE, fig.cap="Figure 4. Counts of playgrounds with a certain number of missing target values. Playgrounds missing less than two observations are excluded from this plot."}
...
Ten different kinds of models were pursued during this iteration. Table 3 shows where the work for each model can be located. It should be mentioned that Jupyter notebooks were run on Amazon EC2 to reduce computation time. An `XGBClassifier` was created and its F1 score for the low-count class was 0.99 and that of the high-count class was 0.85. Given the skewness in the data, poisson regression was used to predict `unacast_session_count` for the high-count data. However, its validation RMSE was 1.22287e+73. Other generalized linear models suitable for count data were considered (e.g. negative binomial); however, the algorithm would not converge and model coefficients could not be obtained. On the other hand, regression models build for the low-count data using `LinearRegression` and `XGBRegressor` had promising validation RMSE values of 54 and 35, respectively.
...
We also considered a tiered approach. This model consisted of a classifier which would predict an observation to be either low count or high count (i.e. below or above 300 sessions). Based on that decision, a prediction would be made using a regressor that was trained on low-count or high-count data.
